...
func devBuilder() *builder.Service {
	return builder.NewService(builder.DefaultConfig("dev", "", ""))
}

func buildBinaries(ctx context.Context, target, version, goos, goarch string, race bool) (*builder.BuildSummary, error) {
	summary, err := devBuilder().BuildWithResults(ctx, builder.BuildOptions{
		Target:  target,
		Version: version,
		GOOS:    goos,
		GOARCH:  goarch,
...
func newLintCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result := devBuilder().LintWithResult(ctx, config)
...
func newFormatCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := devBuilder().FormatCode(ctx, useGofumpt, args...); err != nil {
...
func newCleanCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, err := devBuilder().CleanArtifacts(ctx, builder.CleanOptions{
				All:       all,
				Cache:     cache,
				ModCache:  modCache,
				TestCache: testCache,
...
func newModCommand() *cobra.Command {
	...
	modCmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := devBuilder().RunModuleMaintenance(context.Background(), "tidy"); err != nil {
				return err
			}
	...
	modCmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := devBuilder().RunModuleMaintenance(context.Background(), "download"); err != nil {
				return err
			}
	...
	modCmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := devBuilder().RunModuleMaintenance(context.Background(), "verify"); err != nil {
				return err
			}
	...
	modCmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := devBuilder().RunModuleMaintenance(context.Background(), "vendor"); err != nil {
				return err
			}
	...
	modCmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return devBuilder().RunModuleMaintenance(context.Background(), "graph")
		},
	})
	...
	modCmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return devBuilder().RunModuleMaintenance(context.Background(), "why", args...)
		},
	})
...
func newCheckCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			summary, err := service.RunQualityChecks(ctx, builder.QualityOptions{
				Format:     true,
				Lint:       true,
				Test:       true,
				TestConfig: config,
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if installPath != "" {
				...
				result, err = service.InstallWithResult(ctx, builder.InstallOptions{Path: installPath, SkipBuild: true})
			} else {
				fmt.Printf("📦 Installing...")
				result, err = service.InstallWithResult(ctx, builder.InstallOptions{SkipBuild: true})
			}
...
func newReleaseCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			summary, err := devBuilder().ReleaseWithResults(ctx, version)
			for _, result := range summary.Results {
				if result.Success {
					fmt.Printf("✅ %s\n", result.String())
				} else {
